...
add_response /\A#{self.tc}join\s(#\w+)\s#{self.password}/ do |m,u,c|
  @server.join(m[1]) if u == self.admin
  nil
end

add_response /\A#{self.tc}part\s(#\w+)\s#{self.password}/ do |m,u,c|
  @server.part(m[1]) if u == self.admin
  nil
end

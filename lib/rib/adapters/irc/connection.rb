...
class RIB::Adapters::IRC::Connection
  ...
  # @param hostname [String] hostname or IP address of the server
  # @param nick     [String] nickname to use
  # @param options  [Hash]   optional options to iuse for the connection

  def initialize(hostname, nick, log_path, options = Hash.new)
    @hostname = hostname
    @nick     = nick
    @log_path = log_path
  ...
  def receive
    ...
    when 'PING'
      send_message("PONG #{msg.params.first}")
      receive
    when 'PONG'
      @mutex.synchronize { @last_pong = msg.data }
      receive
    else
      ctcp?(msg) ? receive : msg
    end
  end
  ...
  def connect
    tcp_socket  = TCPSocket.new @hostname, @options[:port]
  ...
  def receive_message
    ...
    if msg.empty?
      ...
      irclog(msg)
      RIB::Adapters::IRC::Message.parse(msg)
    end
  end
